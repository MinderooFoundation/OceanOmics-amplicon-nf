process NESTER_FILTER {
    ...
    controls     <- rownames(SAM[SAM\$sample_type == "negative_control" | SAM\$sample_type == "NTC_control", ])
    ...
    stats_conn <- file(paste0($prefix, "_nester_stats_NTCs.txt"))
    writeLines(stats_vector, stats_conn)
    close(stats_conn)

    write.table(final_taxa, paste0($prefix, "_final_taxa_filtered_NTCs.tsv"), sep = "\t")
    ...
    saveRDS(physeq, file = paste0($prefix, "_phyloseq_filtered_NTCs.rds"))
    ...
    controls     <- rownames(SAM[! SAM\$sample_type == "positive_control" & ! SAM\$sample_type == "ITC_control" & ! SAM\$sample_type == "sample", ])
    ...
    stats_conn <- file(paste0($prefix, "_nester_stats_nonITCs.txt"))
    writeLines(stats_vector, stats_conn)
    close(stats_conn)

    write.table(final_taxa, paste0($prefix, "_final_taxa_filtered_nonITCs.tsv"), sep = "\t")
    ...
    saveRDS(physeq, file = paste0($prefix, "_phyloseq_filtered_nonITCs.rds"))
    ...
    controls	<- rownames(SAM[SAM\$sample_type == "negative_control" | SAM\$sample_type == "NTC_control" | SAM\$sample_type == "EB_control" | SAM\$sample_type == "WC_control", ])
    ...
    stats_conn <- file(paste0($prefix, "_nester_stats_NTCsWCsEBs.txt"))
    writeLines(stats_vector, stats_conn)
    close(stats_conn)

    write.table(final_taxa, paste0($prefix, "_final_taxa_filtered_NTCsWCsEBs.tsv"), sep = "\t")
    ...
    saveRDS(physeq, file = paste0($prefix, "_phyloseq_filtered_NTCsWCsEBs.rds"))
    ...
    controls	<- rownames(SAM[SAM\$sample_type == "negative_control" | SAM\$sample_type == "NTC_control" | SAM\$sample_type == "DI_control" | SAM\$sample_type == "EB_control", ])
    ...
    stats_conn <- file(paste0($prefix, "_nester_stats_NTCsDIsEBs.txt"))
    writeLines(stats_vector, stats_conn)
    close(stats_conn)

    write.table(final_taxa, paste0($prefix, "_final_taxa_filtered_NTCsDIsEBs.tsv"), sep = "\t")
    ...
    saveRDS(physeq, file = paste0($prefix, "_phyloseq_filtered_NTCsDIsEBs.rds"))
