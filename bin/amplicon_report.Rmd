...
if (file.exists("final_stats.tsv") &&
    ...
    final_count    <- sum(final_stats$num_seqs) / 2
...
```{r sample_stats_sample_counts, echo=FALSE}
title_sample_counts     <- "Density Plots"
subtitle_sample_counts  <- "Lets look at density plots of read counts
                        after demultiplexing and trimming."
text_sample_counts      <- c(
    "```{r intext_density, results='asis', echo=FALSE}\n",
    "ggplot(final_stats, aes(x=num_seqs)) +
    geom_histogram(aes(y=after_stat(count)),
                    colour='red', fill='white', bins=30) +
...
knit_list_sample_counts <- get_knit_list(title_sample_counts,
                                    subtitle_sample_counts,
                                    text_sample_counts)
```
`r if (stats_bool) knitr::knit(text = unlist(knit_list_sample_counts))`
