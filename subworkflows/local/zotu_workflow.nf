...
include { COUNTOFSEQ as \
          COUNTOFMERGED;
          COUNTOFSEQ as \
          COUNTOFDEREP;
          COUNTOFSEQ as \
          COUNTOFDENOISE          } from '../../modules/local/custom/countofseq/main'
...
include { ZOTUTRACKREADS          } from '../../modules/local/custom/zotutrackreads/main'
...
workflow ZOTU_WORKFLOW {
    ...
    // Count number of sequences for each sample
    COUNTOFMERGED ( 
        "merged",
        FASTQTOFASTA.out.reads 
    ) 
    ...
    COUNTOFDENOISE ( 
        "denoised",
        ch_denoise_fasta.map{ it = it[1] }
    )     
    ch_versions = ch_versions.mix(COUNTOFDENOISE.out.versions)

    // Track reads through stages
    ZOTUTRACKREADS (
        ch_prefix,
        COUNTOFMERGED.out.counts,
        COUNTOFDEREP.out.counts,
        COUNTOFDENOISE.out.counts
    )
    ...
    track_reads     = ZOTUTRACKREADS.out.png // channel: [ zotu_track_reads.png ]
